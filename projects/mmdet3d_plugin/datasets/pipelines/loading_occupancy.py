...
class LoadOccupancy(object):
    ...
    def __init__(self, to_float32=True, occ_path=None, grid_size=[512, 512, 40], pc_range=[-51.2, -51.2, -5.0, 51.2, 51.2, 3.0], unoccupied=0, 
                 use_separate_classes=False, use_background_classes=False, time_history_field=2, time_future_field=2, gt_resize_ratio=1, use_fine_occ=False, test_mode=False):
        '''
        ...
        self.use_background_classes = use_background_classes
    ...
    def get_seq_occ(self, results):
        ...
        for count in range(sequence_length):
            ...
            if not self.use_separate_classes:
                if not self.use_background_classes: # only GMO
                    # 1. fine-grained GMO
                    processed_label[torch.isin(processed_label, torch.tensor([0,1,8,11,12,13,14,15,16]))] = 0   # ignore background
                    processed_label[torch.isin(processed_label, torch.tensor([2,3,4,5,6,7,9,10]))] = 1          # GMO=1
                elif self.use_background_classes:   # GMO + GSO
                    # 1. fine-grained GMO, fine-grained GSO
                    tmp_id = 255
                    processed_label[torch.isin(processed_label, torch.tensor([2,3,4,5,6,7,9,10]))] = tmp_id # GMO->255
                    processed_label[torch.isin(processed_label, torch.tensor([1,8,11,12,13,14,15,16]))] = 2 # GSO=2
                    processed_label[processed_label == tmp_id] = 1                                          # GMO=1
    ...
    def __call__(self, results):
        if not results['occ_load_flag']:   # only load current frame
            return results
