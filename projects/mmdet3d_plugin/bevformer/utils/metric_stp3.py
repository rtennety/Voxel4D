...
class PlanningMetric():
    ...
    def evaluate_single_coll(self, traj, segmentation, input_gt):
        ...
        rc = np.concatenate([rr[:,None], cc[:,None]], axis=-1)  # 32,2  32*[H_bev, W_bev]
        ...
        trajs_ = trajs_ / self.dx.to(trajs.device)  # Lout,1,2
        trajs_ = trajs_.cpu().numpy() + rc # Lout,32,2    [H_bev+delta, W_bev+delta]
